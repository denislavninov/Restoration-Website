  ...
  return (
    ...
    >
      ...
      <nav className="absolute top-0 left-0 right-0 p-4 flex items-center h-[100px]  z-10 md:justify-evenly justify-start">
        ...
        <button className="hidden md:block bg-white hover:bg-black hover:text-white text-gray-800 text-md px-4 py-2 rounded-xs" onClick={scrollToContact}>GET IN TOUCH</button>
      </nav>
      <div className="absolute top-100 ml-10 z-10" style={{ color: 'var(--foreground)' }}>
        {isHomePage && (
          <>
            ...
            <div className="mt-4">
              <button className="bg-white hover:bg-black hover:text-white text-gray-800 text-lg px-4 py-2 rounded-xs mr-2">Scroll Down  </button>
            </div>
          </>
        )}
      </div>
